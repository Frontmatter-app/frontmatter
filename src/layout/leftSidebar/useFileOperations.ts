...
export function useFileOperations() {
  const { documents, cloudFolders, openDocument, closeDocument, currentDocumentId, workspacePath, directoryTree, refreshDirectoryTree, openFileFromPath, openExternalDocument, createDocument, createFileInWorkspace, createFolderInWorkspace, deleteFileOrFolderFromWorkspace, syncToCloud, unsyncFromCloud, setOfflineEnabled } = useWorkspace();
  const { user } = useAuth();
  ...
  const handleOpenFile = async (path: string) => {
    if (!(await guardTeamAuth(isTeamContext, user, 'access team files'))) return;
    openFileFromPath(path);
  };
  const handleOpenCloudDoc = (docId: string) => openDocument(docId);
  const handleOpenExternalDocument = async () => {
    if (!(await guardTeamAuth(isTeamContext, user, 'add files to a team workspace'))) return;
    if (!(await guardTeamPermission(isTeamContext, teamPerms.addToTeam, 'add files to the team'))) return;
    openExternalDocument();
  };
  const handleNewRootFile = async () => {
    if (!(await guardTeamAuth(isTeamContext, user, 'create files in a team workspace'))) return;
  ...
  const handleNewRootFolder = async () => {
    if (!(await guardTeamAuth(isTeamContext, user, 'create folders in a team workspace'))) return;
    ...
    if (!name?.trim()) return;
    try { await createFolderInWorkspace(workspacePath, name.trim()); } catch (e: any) { await showAlertDialog('Create Folder Failed', `Could not create folder: ${e}`); }
  };
  const cloudFolderPathFromParent = (parentDir: string, name: string) => {
    const parentPath = isCloudVirtualDir(parentDir) ? parentDir.replace(CLOUD_DIR_PREFIX, '').replace(/^root\/?/, '') : workspacePath && parentDir.startsWith(workspacePath) ? parentDir.slice(workspacePath.length).replace(/^\/+/, '') : '';
    return `${parentPath}/${name}`.replace(/\/+/g, '/').replace(/^\/+|\/+$/g, '');
  };
  const createCloudFolderAtPath = async (cloudPath: string) => {
    if (!user || !teamId) { await showAlertDialog('Authentication Required', 'Cannot create cloud folder: user or team context not available.'); return; }
    const normalizedPath = cloudPath.replace(/\/+/g, '/').replace(/^\/+|\/+$/g, '');
    if (!normalizedPath) return;
    try { await createCloudFolder(normalizedPath, user.id, teamId); } catch (e: any) { await showAlertDialog('Create Folder Failed', `Could not create cloud folder: ${e.message || e}`); }
  };
  const createCloudDocumentAtPath = async (cloudPath: string) => {
    if (!user || !teamId) { await showAlertDialog('Authentication Required', 'Cannot create cloud document: user or team context not available.'); return; }
    ...
    try {
      ...
      await pushCloudDocument({ id: newId, title, content, cloud_path: normalizedPath, stage: 'draft', focus_mode: false } as DocumentMeta, user.id, teamId, normalizedPath);
      await invoke('set_cloud_sync', { id: newId, cloudId: newId, cloudPath: normalizedPath });
      await refreshDirectoryTree();
      openDocument(newId);
    } catch (e: any) { await showAlertDialog('Error', `Failed to create cloud document: ${e}`); }
  };
  const handleCreateFile = async (parentDir: string, name: string) => {
    if (!(await guardTeamAuth(isTeamContext, user, 'create files in a team workspace'))) return;
    if (!(await guardTeamPermission(isTeamContext, teamPerms.createFiles, 'create files'))) return;
    if (isTeamContext) {
      const relativeParent = workspacePath && parentDir.startsWith(workspacePath) ? parentDir.slice(workspacePath.length).replace(/^\/+/, '') : '';
      const fullCloudPath = `${relativeParent}/${name}`.replace(/\/+/g, '/').replace(/^\//, '').replace(/\/$/, '');
      await createCloudDocumentAtPath(fullCloudPath || name);
      return;
    }
    try { await createFileInWorkspace(parentDir, name); } catch (e: any) { await showAlertDialog('Create File Failed', `Could not create file: ${e}`); }
  };
  const handleCreateFolder = async (parentDir: string, name: string) => {
    if (!(await guardTeamAuth(isTeamContext, user, 'create folders in a team workspace'))) return;
    if (!(await guardTeamPermission(isTeamContext, teamPerms.createFolders, 'create folders'))) return;
    if (isTeamContext) { const cloudPath = cloudFolderPathFromParent(parentDir, name); await createCloudFolderAtPath(cloudPath); return; }
    try { await createFolderInWorkspace(parentDir, name); } catch (e: any) { await showAlertDialog('Create Folder Failed', `Could not create folder: ${e}`); }
  };
  ...
  const handleDeleteFileOrFolder = async (path: string, name: string) => {
    if (!(await guardTeamAuth(isTeamContext, user, 'manage files in a team workspace'))) return;
    if (!(await guardTeamPermission(isTeamContext, teamPerms.deleteFiles, 'delete files'))) return;
    if (isCloudOnlyNode(path)) {
      ...
      if (confirmed) { await deleteCloudDocument(docId).catch(console.error); if (currentDocumentId === docId) closeDocument(docId); }
      return;
    }
    const confirmed = await showConfirmDialog('Delete File/Folder', `Delete "${name}"? This action cannot be undone.`);
    if (confirmed) deleteFileOrFolderFromWorkspace(path);
  };
  const handleRename = async (path: string, newName: string) => {
    if (!(await guardTeamAuth(isTeamContext, user, 'rename files in a team workspace'))) return;
    if (!(await guardTeamPermission(isTeamContext, teamPerms.renameFiles, 'rename files'))) return;
    if (isCloudVirtualDir(path)) { await showAlertDialog('Not Allowed', 'Cloud directories cannot be renamed directly. Rename the files inside instead.'); return; }
    if (isCloudOnlyNode(path)) {
      const docId = path.replace('__cloud__:', '');
    ...
    try {
      ...
      parts.pop();
      await invoke('move_or_rename_on_disk', { oldPath: path, newPath: `${parts.join('/')}/${newName}` });
      await refreshDirectoryTree();
    } catch (e: any) { await showAlertDialog('Rename Failed', `Rename failed: ${e}`); }
  };
  const handleMove = async (oldPath: string, newPath: string) => {
    if (!(await guardTeamAuth(isTeamContext, user, 'move files in a team workspace'))) return;
    if (!(await guardTeamPermission(isTeamContext, teamPerms.renameFiles, 'rename files'))) return;
    if (isCloudOnlyNode(oldPath) || isCloudVirtualDir(oldPath)) return;
    try { await invoke('move_or_rename_on_disk', { oldPath, newPath }); await refreshDirectoryTree(); } catch (e: any) { await showAlertDialog('Move Failed', `Move failed: ${e}`); }
  };
  const handleRevealInFolder = async (path: string) => {
    if (isCloudOnlyNode(path) || isCloudVirtualDir(path)) return;
    try { await invoke('reveal_in_folder', { path }); } catch (e: any) { await showAlertDialog('Error', `Failed to reveal in Finder: ${e}`); }
  };
  const handleAddToTeam = async (path: string) => {
    if (!(await guardTeamPermission(isTeamContext, teamPerms.addToTeam, 'add files to the team'))) return;
    try { await syncToCloud(path); } catch (e: any) { await showAlertDialog('Sync Failed', e.message || 'Failed to add to team.'); }
  };
  return {
